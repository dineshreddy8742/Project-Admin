...
import { Edit2, Check, X, Trash2, Plus, Megaphone } from 'lucide-react';
        ...
        <table className="w-full">
          <thead className="bg-muted">
            <tr>
              ...
              <th className="p-4 text-left text-sm font-medium text-muted-foreground">Top Seller</th>
              <th className="p-4 text-left text-sm font-medium text-muted-foreground">New Arrival</th>
              ...
              <tr key={product?.id} className="border-b border-border hover:bg-muted/50">
                ...
                <td className="p-4">
                  {editingId === product?.id ? (
                    ...
                    <span className="text-sm text-foreground">{product?.discountPercentage || 0}%</span>
                  )}
                </td>
                <td className="p-4">
                  {editingId === product?.id ? (
                    <Input
                      type="checkbox"
                      checked={editData?.isTopSeller}
                      onChange={(e) => setEditData({...editData, isTopSeller: e?.target?.checked})}
                    />
                  ) : (
                    <span className="text-sm text-foreground">{product?.isTopSeller ? 'Yes' : 'No'}</span>
                  )}
                </td>
                <td className="p-4">
                  {editingId === product?.id ? (
                    <Input
                      type="checkbox"
                      checked={editData?.isNewArrival}
                      onChange={(e) => setEditData({...editData, isNewArrival: e?.target?.checked})}
                    />
                  ) : (
                    <span className="text-sm text-foreground">{product?.isNewArrival ? 'Yes' : 'No'}</span>
                  )}
                </td>
                <td className="p-4">
