  ...
  useEffect(() => {
    const loadAnalytics = async () => {
      ...
      try {
        ...
        setLoadError('Could not load live analytics. Showing mock data.');
        setAnalyticsData(null); // could still show last data if desired
      } finally {
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className={`analytics-loading border rounded-lg p-8 bg-gradient-to-r from-purple-50 to-blue-50 border-purple-200 ${className}`}>
        <div className="flex items-center justify-center space-x-3">
          ...
          <span className="text-red-700 font-medium">{loadError}</span>
        </div>
      </div>
    );
  }
    ...
    <div className={`marketplace-analytics ${className}`}>
      ...
      <div className="mb-6">
        ...
        <div className="border-b border-gray-200">
          <div className="flex flex-wrap space-x-0 gap-y-1">
            {[
              ...
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 min-w-16 rounded focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-600 text-sm font-medium border-b-2 transition-colors ${
                  activeTab === tab.id
